...
BM = bioModel(100,0.1,0.3,3e-3);
BM.addSolidComponent('Na',1,1);
BM.addSolidComponent('Na2',0,0);
BM.addFluidComponent('NaAq',0,0);
BM.addFluidComponent('CaAq',0,0);
BM.addSolidComponent('Ca',1,1);
...
BM.addTransferFunction('Ca->CaAq','Ca','CaAq',1,1,1e-2);
BM.addTransferFunction('CaAq->Ca','CaAq','Ca',1,1,1e-2);
BM.addScalingFunctionToTransferFunction('Ca->CaAq','CaAq',-2e2,0,0,1,0,1);

%BM.addTransferFunction('Ca->none','Ca','NULL',1,1,1e-2);

%BM.addScalingFunctionToTransferFunction('Na->NaAq','Ca',1,2,3,4,5,6);

aviobj = VideoWriter('MultiComponentBenchmarkErosion.avi','Motion JPEG AVI');
open(aviobj);
